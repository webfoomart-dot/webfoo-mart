...
export default function HomePage() {
  ...
  const renderProductCard = (product: any, isHorizontalMode: boolean = false, isCatOpen: boolean = true) => {
    ...
    return (
      <motion.div 
        ...
        className={`cursor-pointer bg-card/50 dark:bg-white/5 p-3 rounded-[1.5rem] border border-border dark:border-white/10 flex flex-col gap-3 relative transition-all duration-300 hover:border-[#00FFFF]/40 hover:bg-black/5 dark:hover:bg-white/10 group ${itemLocked ? 'opacity-60 grayscale cursor-not-allowed pointer-events-none' : ''} ${isHorizontalMode ? 'min-w-[160px] max-w-[160px] sm:min-w-[190px] sm:max-w-[190px] snap-start shrink-0' : ''}`}
      >
        <div className="relative h-36 sm:h-44 w-full rounded-xl overflow-hidden flex items-center justify-center p-0 border border-border dark:border-white/5 bg-black/5 dark:bg-black/20">
          <Image 
        ...
        <div className="flex items-center justify-between mt-1 pt-3 border-t border-border dark:border-white/10" onClick={(e) => e.stopPropagation()}>
          ...
          {cartItem ? (
            ...
            <Button 
              ...
              size="sm" 
              className="h-9 bg-[#CCFF00] text-black font-black text-[11px] uppercase tracking-widest rounded-lg px-4 hover:bg-[#CCFF00]/80 disabled:bg-muted disabled:text-muted-foreground shadow-[0_0_15px_rgba(204,255,0,0.15)] pointer-events-auto"
            >
              ADD
            </Button>
          )}
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-[#00FFFF]/30 pb-32 pt-24 relative transition-colors duration-300">
      <Header />
      ...
      <main className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
        <AnimatePresence mode="wait">
          {!storeConfig ? (
            ...
            <motion.div key="open" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative h-40 sm:h-52 w-full overflow-hidden rounded-[1.5rem] bg-black/5 dark:bg-[#0A0A0A] border-2 border-[#00FFFF]/40 shadow-[0_0_30px_rgba(0,255,255,0.1)] group flex items-center px-6 sm:px-8">
        ...
        <div className="space-y-4">
          <div className="relative mt-2">
            ...
            <Input type="search" placeholder="Search for snacks, drinks & more..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="h-14 pl-14 pr-6 bg-card dark:bg-white/5 border-border dark:border-white/10 rounded-full text-sm text-foreground focus-visible:border-[#00FFFF] shadow-inner w-full" />
          </div>
        </div>
        ...
        {(!products || products.length === 0) ? (
          ...
          <div className="py-20 text-center border border-dashed border-border rounded-[1.5rem] bg-card dark:bg-white/5">
        ...
        {storeConfig && storeConfig.ownerName && (
          <div className="mt-16 pt-8 border-t border-border">
            <div className="max-w-md mx-auto bg-card/50 dark:bg-black/40 p-6 rounded-[2rem] border border-border dark:border-white/5 text-center flex flex-col items-center gap-4 relative overflow-hidden group">
              ...
              {storeConfig.ownerPhoto ? (
                <div className="relative w-20 h-20 rounded-full border-2 border-[#CCFF00]/30 p-1 bg-background overflow-hidden z-10 group-hover:scale-105 transition-transform duration-300">
              ...
              <div className="flex gap-4 w-full mt-2 z-10">
                {storeConfig.ownerPhone && (
                  <a href={`tel:${storeConfig.ownerPhone}`} className="flex-1">
                    <Button variant="outline" className="w-full h-10 border-border hover:border-[#CCFF00]/50 hover:bg-[#CCFF00]/10 hover:text-[#CCFF00] text-muted-foreground transition-all">
                ...
                {storeConfig.ownerEmail && (
                  <a href={`mailto:${storeConfig.ownerEmail}`} className="flex-1">
                    <Button variant="outline" className="w-full h-10 border-border hover:border-[#00FFFF]/50 hover:bg-[#00FFFF]/10 hover:text-[#00FFFF] text-muted-foreground transition-all">
              ...
              <SheetHeader className="p-6 border-b border-border bg-card/50 shrink-0">
                <div className="text-left flex flex-col gap-3">
                  <div className="flex items-center gap-2">
                    <Badge variant="outline" className={`bg-transparent border-border ${activeOrder.status === 'Pending' ? 'text-orange-500' : activeOrder.status === 'Preparing' ? 'text-[#00FFFF]' : 'text-[#CCFF00]'} text-[9px] uppercase tracking-widest font-black px-2 py-0.5`}>
                      Live Status
                    </Badge>
                  </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 relative">
                ...
                <div className="space-y-12 relative z-10">
                  ...
                  <div className="flex items-start gap-6">
                    ...
                    <div className="pt-2"><h4 className={`font-bold text-lg leading-none transition-colors ${activeOrder.status === 'Preparing' ? 'text-[#00FFFF]' : activeOrder.status === 'In Transit' ? 'text-foreground' : 'text-muted-foreground/30'}`}>Food is being prepared</h4><p className="text-xs text-muted-foreground mt-1">Our chef is on it.</p></div>
                  </div>

                  <div className="flex items-start gap-6">
                    <div className={`relative w-12 h-12 rounded-full bg-card border-[3px] flex items-center justify-center shrink-0 z-10 transition-all duration-500 overflow-hidden ${activeOrder.status === 'In Transit' ? 'border-[#CCFF00] text-[#CCFF00] shadow-[0_0_20px_rgba(204,255,0,0.2)] scale-110' : 'border-border text-muted-foreground/30 scale-100'}`}>
                    ...
                    <div className="pt-2"><h4 className={`font-bold text-lg leading-none transition-colors ${activeOrder.status === 'In Transit' ? 'text-[#CCFF00]' : 'text-muted-foreground/30'}`}>On the Way</h4><p className="text-xs text-muted-foreground mt-1">Rider is heading to your location.</p></div>
                  </div>
