...
export default function NotificationsPage() {
  ...
  const getIcon = (type: string) => {
    switch(type) {
      case 'package': return <Package className="w-6 h-6 text-primary" />
      case 'success': return <CheckCircle2 className="w-6 h-6 text-secondary dark:text-[#CCFF00]" />
      case 'message': return <MessageSquare className="w-6 h-6 text-primary" />
      default: return <Clock className="w-6 h-6 text-foreground" />
    }
  }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-primary/30 transition-colors duration-300">
      ...
      <main className="container mx-auto pb-40 pt-24 px-4 max-w-2xl">
        ...
        >
          <Link href="/">
            <Button variant="ghost" size="icon" className="rounded-full hover:bg-black/10 dark:hover:bg-white/10 text-primary">
              <ArrowLeft className="h-6 w-6" />
            </Button>
          </Link>
          <h1 className="text-3xl font-black tracking-tighter flex items-center gap-2 uppercase text-primary">
            Updates
          </h1>
        </motion.div>
          ...
          {!user ? (
            <motion.div key="unauth" className="flex justify-center mt-10">
              <Empty className="glass-strong border-primary/20 py-16 w-full max-w-md">
                <EmptyContent>
                  <div className="bg-primary/10 p-4 rounded-full mb-4 shadow-[0_0_20px_rgba(0,139,139,0.2)] dark:shadow-[0_0_20px_rgba(0,255,255,0.2)]">
                    <BellOff className="w-12 h-12 text-primary" />
                  </div>
                  <EmptyTitle className="text-2xl font-black uppercase tracking-tighter text-foreground">Login Required</EmptyTitle>
                  ...
                  <Button asChild className="mt-4 bg-secondary text-secondary-foreground dark:text-black font-black hover:bg-secondary/90 rounded-xl px-8 h-12"><Link href="/profile">GO TO LOGIN</Link></Button>
                </EmptyContent>
              </Empty>
            </motion.div>
            ...
            <motion.div key="empty" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="flex justify-center mt-10">
              <Empty className="glass-strong border-primary/20 py-16 w-full max-w-md">
                <EmptyContent>
                  <div className="bg-primary/10 p-4 rounded-full mb-4 shadow-[0_0_20px_rgba(0,139,139,0.2)] dark:shadow-[0_0_20px_rgba(0,255,255,0.2)]"><BellOff className="w-12 h-12 text-primary" /></div>
                  <EmptyTitle className="text-2xl font-black uppercase tracking-tighter text-foreground">No Updates</EmptyTitle>
                  ...
                  <Button asChild className="mt-4 bg-secondary text-secondary-foreground dark:text-black font-black hover:bg-secondary/90 rounded-xl px-8 h-12"><Link href="/">SHOP NOW</Link></Button>
                </EmptyContent>
              </Empty>
            </motion.div>
          ) : (
              ...
              {notifications.map((note) => (
                <motion.div key={note.id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ type: "spring" as const, stiffness: 300, damping: 30 }}>
                  <Card 
                    ...
                    className={`glass-strong transition-colors relative overflow-hidden ${note.unread ? 'border-primary/40' : 'border-border'} ${note.isAdminNote ? 'cursor-pointer' : ''}`}
                  >
                    {note.unread && <div className="absolute left-0 top-0 bottom-0 w-1 bg-primary shadow-[0_0_10px_rgba(0,139,139,1)] dark:shadow-[0_0_10px_#00FFFF]" />}
                    <CardContent className="p-4 sm:p-5 flex gap-4 items-start bg-card/50 dark:bg-transparent">
                      <div className={`p-3 rounded-xl shrink-0 ${note.icon === 'success' ? 'bg-secondary/10 dark:bg-[#CCFF00]/10' : 'bg-primary/10'}`}>{getIcon(note.icon)}</div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start gap-2 mb-1">
                          <h4 className="font-bold text-foreground text-sm sm:text-base leading-tight">{note.title}</h4>
