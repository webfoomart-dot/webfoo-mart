...
export default function NotificationsPage() {
  ...
  const { orders, notifications: adminNotes, markNotificationRead } = useAppStore() as any
  ...
  const notifications = React.useMemo(() => {
    ...
    const formattedAdminNotes = (adminNotes || []).map((note: any) => ({
      id: note.id,
      title: "WebFoo Update 📢",
    ...
    return visible
  }, [orders, adminNotes, hiddenNotifs])
              ...
              <Empty className="glass-strong border-[#00FFFF]/20 py-16 w-full max-w-md">
                <EmptyContent>
                  ...
                  <EmptyDescription className="text-muted-foreground mb-4">
                    Looks like you haven't placed any orders yet. Start shopping to see live updates here!
                  </EmptyDescription>
                  <Button asChild className="mt-4 bg-[#CCFF00] text-black font-black hover:bg-[#CCFF00]/90 rounded-xl px-8 h-12">
              ...
              {notifications.map((note) => (
                ...
                >
                  ...
                  >
                    ...
                    {note.unread && (
                      <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#00FFFF] shadow-[0_0_10px_#00FFFF]" />
                    )}

                    <CardContent className="p-4 sm:p-5 flex gap-4 items-start">
                      {/* Icon Container */}
                      <div className={`p-3 rounded-xl shrink-0 ${note.icon === 'success' ? 'bg-[#CCFF00]/10' : 'bg-[#00FFFF]/10'}`}>
                        {getIcon(note.icon)}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start gap-2 mb-1">
                          ...
                          <div className="flex items-center gap-3 shrink-0 mt-0.5">
                            ...
                            <button 
                              onClick={(e) => handleHideNotification(e, note.id)}
